...
员工在飞书里发送 `/start` 打开操作卡片，或发送 `申请授权 1.2.3.4 2h`，机器人调用 frps 管理 API 更新授权并回复确认。
...
| 消息来源 | 应用可用范围内的员工私聊，或群内 @机器人 |
| 使用范围 | 默认由飞书开放平台的应用可用范围限制；也可设置 `allowAllUsers=false` 并用 `adminChatID` 只允许指定群 |
| 指令集 | 见 3.4 |
...
[bot]
allowAllUsers = true
# allowAllUsers=false 时必须指定允许使用机器人的群
# adminChatID = "oc_xxxxxxxx"
defaultTTL = "2h"                     # 「申请授权」不带时长时使用
```
...
| `申请授权 1.2.3.4 2h` | 无 | 指定公网 IP 和有效期，支持 `30m` / `2h` / `1d` 等格式 |
| `我的授权` | 无 | 列出本人全部有效授权及到期时间 |
| `撤销授权 1.2.3.4` | 无 | 撤销本人的指定授权 |

解析失败时回复用法提示。IP 格式校验在 bot 与 frps API 双侧进行。

## 4. 非功能需求

- **安全性**：管理 API 走 Basic Auth 且仅允许本机或管理内网访问；bot 仅面向飞书应用可用范围内的员工；白名单变更记录 IP、操作者、消息、时间和结果。
...
| 阶段二：上线 | 公司飞书租户 | 公司租户新建自建应用，走企业管理员审批发布；替换 bot 配置中的 AppID/AppSecret，设置公司成员可用范围后完成迁移 |

注意：如使用指定群模式，迁移时必须换成公司租户内的群 ID。
...
6. 应用可用范围外或指定群外的指令不被受理；管理 API 无鉴权访问返回 401；
