...
func TestParseCurrentCommands(t *testing.T) {
	cases := []struct {
		...
		{"grant ipv6", "申请授权 2001:db8::1", ActionAdd, "2001:db8::1", 0, false},
		{"reject loopback", "申请授权 127.0.0.1", ActionHelp, "", 0, true},
		{"reject private", "申请授权 10.0.0.1", ActionHelp, "", 0, true},
		...
		{"apply", "授权", ActionApply, "", 0, false},
		{"apply slash", "/auth", ActionApply, "", 0, false},
		{"apply rejects args", "授权 1.2.3.4", ActionHelp, "", 0, true},
